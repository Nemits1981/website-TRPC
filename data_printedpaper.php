...
// Query untuk mengambil data dari tabel data_printedpaper
$sql_printedpaper = "SELECT NO, PAPER_TYPE, PRINTING, QTY, UNIT FROM data_printedpaper";
$result_printedpaper = $conn->query($sql_printedpaper);
?>
...
<head>
    ...
    <title>Data Printed Paper</title>
    <style>
    body{
...
<body>
    ...
    <div class="filter-container"> 
        ...
        <label for="filter-printing"></label>
        <select id="filter-printing">
    ...
    <table>
        ...
        <?php
            $printings = [];
            $no = 1;
            if ($result_printedpaper->num_rows > 0) {
                while($row = $result_printedpaper->fetch_assoc()) {
                    ...
                    $no++;
                    // Collect unique printings
                    if (!in_array($row['PRINTING'], $printings)) {
                        $printings[] = $row['PRINTING'];
                    }
                }
            } else {
    ...
    <script>
        ...
        window.addEventListener('DOMContentLoaded', function() {
            ...
            let printings = <?php echo json_encode($printings); ?>;

            // Populate dropdowns
            populateDropdown('filter-printing', printings);

            updateCountbox();
        });

        // Fungsi untuk memfilter tabel
        // Fungsi untuk memperbarui nomor urut setelah filter
        function updateRowNumbers() {
            let visibleRows = document.querySelectorAll('tbody tr:not([style*="display: none"])'); // Ambil hanya baris yang terlihat
        ...
        function filterTable() {
            ...
            let printingFilter = document.getElementById('filter-printing').value.toLowerCase();
            let qtySort = document.getElementById('filter-qty').value;
            let rows = Array.from(document.querySelectorAll('tbody tr'));

        // Filter berdasarkan Paper Type dan printing
        rows.forEach(row => {
            let paperTypeCell = row.querySelector('td:nth-child(2)');
            let printingCell = row.querySelector('td:nth-child(3)');
            let paperTypeMatch = !paperTypeFilter || (paperTypeCell && paperTypeCell.textContent.toLowerCase().includes(paperTypeFilter));
            let printingMatch = !printingFilter || (printingCell && printingCell.textContent.toLowerCase().includes(printingFilter));

        // Tampilkan baris hanya jika semua filter sesuai
        if (paperTypeMatch && printingMatch) {
            row.style.display = '';
        } else {
    ...
    // Jika memilih sortir Qty
    if (qtySort) {
        rows.sort((a, b) => {
            let qtyA = parseInt(a.querySelector('td:nth-child(4)').getAttribute('data-raw-qty'));
            let qtyB = parseInt(b.querySelector('td:nth-child(4)').getAttribute('data-raw-qty'));
    ...
    function resetFilters() {
        ...
        document.getElementById('filter-printing').value = '';
...
document.getElementById('filter-printing').addEventListener('change', filterTable);
